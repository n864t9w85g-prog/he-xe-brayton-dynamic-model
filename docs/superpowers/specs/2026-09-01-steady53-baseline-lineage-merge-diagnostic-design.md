...
✅ Simulink 官方 API 比较得到：两者共有 53 项参数值差异，其中 40 项的参数名为 `InitialCondition`；进一步按块类型复核后，这 40 项由 39 个积分器和 1 个 `Unit Delay1` 组成。两版共有 40 个同路径积分器，其中 `TAC/rotor/N_rpm_Integrator` 均为 `55090`，其余 39 个积分器初值不同。其余差异包括负荷、TAC 常量、换热参数、散热器参数表达式、模型停止时间和一个增益。根模型另有四个 TAC Goto/From 块。
...
该操作把“初值向量来源”视为一个完整实验变量：执行 40 项逐路径赋值，形成 39 项有效数值变化和 1 项原本相同的转速初值；`Unit Delay1` 不属于该 40 状态向量，保持冻结模型值。不得只挑选其中更接近论文的状态，也不得在运行后调整任何初值。
...
- 候选必须验证 exact 40 路径一一对应，候选 40 状态表达式与根模型逐项相同，且除其中 39 个实际不同的 `InitialCondition` 外没有参数或拓扑变化。
