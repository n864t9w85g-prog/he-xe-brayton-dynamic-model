...
- ✅ 全部记录输出和积分状态在 `400–500 s` 的波动/趋势均通过；状态最大相对
...
- ✅ 本轮重新检查论文 PDF：第 38–39 页（印刷页 23–24）Eq. (2.28)/(2.30)
  分别使用过程平均绝热系数和过程平均定压比热；第 88 页（印刷页 73）表 4.9
  直接给出方案 B `eta_T=0.87`，本轮未沿用任何旧 dynamic 效率结论。
- ✅ 当前透平块使用入口单点 `gamma` 和点 `1/2s` 单点 `cp`。末时刻数值为
...
- ❓ 根因候选：模型单点 `gamma/cp` 与论文 Eq. (2.28)/(2.30) 过程平均量之间
