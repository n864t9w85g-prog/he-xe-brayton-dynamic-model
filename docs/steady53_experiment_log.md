...
### 2026-08-26 H1b：正式物性路径平均 cp 只读实验

- ✅ **人工批准与单变量范围。** H1b 固定采用 H1a-S2 Scheme A 候选
  `phiBar=0.39979002315209694`、`T2s=1089.5635709913104 K`、
  `eta=0.87286960881076081` 与 `T2=1143.6624955393854 K`；Scheme A
  仍只存在于 H1a-S2 的 `phi` 路径。H1b 唯一新增变量是把 H1a 的单点
  `cp1/cp2` 替换为正式 `HeXe_property_simulink(T,P)` 的路径积分平均
  `cpBarActual/cpBarIsentropic`。未修改或调用 Scheme A 作为 H1b 物性。
- ✅ **H1a 点比热基线复现。** H1a 证据的
  `etaCp1Cp2HeldFixed=true` 表明 S2 沿用原 baseline 的 `cp2`，其取值温度为
  `1089.6475181122112 K`，而不是在 Scheme A 的 `T2s` 上重算。正式物性给出
  `cp1=519.65678111047418 J/(kg K)`、
  `cp2=519.65806622669288 J/(kg K)`；按该真实冻结合同复算 H1a-S2
  `T2` 的残差为 `0 K`。若错误地在 Scheme A `T2s` 重算 `cp2`，会产生约
  `5.19e-6 K` 偏差，不能通过固定 `1e-9 K` 基线门。
- ✅ **三条候选均完成积分和隐式求根。** Eq. (2.30) 固定使用
  `cpBarIsentropic/cpBarActual`，根区间为 `[T2s,T1]`，论文目标不进入
  区间、容差或求根方向：
  - `linearEndpointPressure`：`cpBarS=519.6587987339517`、
    `cpBarA=519.65671027004487 J/(kg K)`、比值
    `1.0000040189299311`、`T2=1143.6619213135946 K`、根残差
    `5.3410076361615211e-10 K`；相对 H1a 变化
    `-0.00057422579084231984 K`，剩余目标误差
    `18.338078686405424 K`，解释比例
    `-3.1314282271922326e-05`。
  - `constantP1`：`cpBarS=519.67684721790647`、
    `cpBarA=519.67333875112047 J/(kg K)`、比值
    `1.0000067512926378`、`T2=1143.660906386531 K`、根残差
    `-6.0867932916153222e-10 K`；相对 H1a 变化
    `-0.0015891528544216271 K`，剩余目标误差
    `18.339093613469004 K`，解释比例
    `-8.6661347940491064e-05`。
  - `constantP2`：`cpBarS=519.645275255667`、
    `cpBarA=519.64373524201346 J/(kg K)`、比值
    `1.0000029635951502`、`T2=1143.6623133135543 K`、根残差
    `3.3151081879623234e-10 K`；相对 H1a 变化
    `-0.00018222583116767055 K`，剩余目标误差
    `18.33768668644575 K`，解释比例
    `-9.9373298890836776e-06`。
- ⚠️ **六条 1001 点路径审计。** 三条理想/实际 resolved path 全部满足
  `cp>0`、`cv>0`、`gamma>1`、`rho>0`：线性路径理想/实际的
  `cp` 范围分别为 `[519.65678111047418,519.65968093625816]` 与
  `[519.65373898388862,519.65751331344734]`；恒 P1 为
  `[519.65678111047418,519.70596136850155]` 与
  `[519.65678111047418,519.6961539766964]`；恒 P2 为
  `[519.63648738293847,519.65807349266129]` 与
  `[519.63648738293847,519.65373895490508] J/(kg K)`。各自完整
  `cv/gamma/rho` 最小值、最大值均已写入 CSV/TXT。六条审计均明确标记
  `formalGlobalProof=false` 与
  `finite1001PointAuditNotFormalGlobalProof`，只是有限数值证据。
- ❓ **H1b 数值充分性为否。** 三条候选的 `deltaT2` 全为负，而目标差
  `1162-1143.6624955393854` 为正；因此三条均
  `targetDirectionMatched=false`、`h1bNumericallySufficient=false`。
  H1b 平均比热处理不能解释当前约 `18.34 K` 的偏差，并会极小幅加重偏差。
  该判决只描述数值方向和幅度，不证明任何压力路径的物理正确性。
- ✅ **固定证据与精度治理。** 最终目录严格只含全精度 CSV/TXT：CSV
  SHA-256 为
  `d042cefe400b93c021ab3b11afdbf5e23de14afe64efbea7a037ab594b873ca1`，TXT
  SHA-256 为
  `42344092dd01941668efb6509c6b17aa25cc3acbcbb56febfb3906f339d3879c`。
  首次 CSV 曾由 `writetable` 默认格式缩为约 15 位有效数字；新增精度测试
  RED 后改为逐数值字段 `%.17g`，精确删除并以同一固定输入重建两份派生物。
  旧版本可由提交 `b6b4bde` 恢复；最终二次运行仍以
  `steady53:H1bOutputExists` 拒绝覆盖且两个新 hash 不变。
- ✅ **测试与边界复核。** 新 H1b 纯求解器/analyzer 测试 `22/22`，H1a/H2a
  加 H1b 聚焦回归 `74/74`，扩展 no-SLX 回归 `171/171`，均为
  `0 Failed, 0 Incomplete`；完整 `tests/steady53` 仅 discovery `198` 项，
  未运行可能触及 SLX 的完整套件。四个新增 MATLAB 文件 `checkcode=0`，禁止
  模型 API 扫描为空，全部受保护 hash 与 archive peeled commit 保持固定。
  与 MATLAB 实现分离的 Python/静态证据复审结果为
  `Critical=0, Important=0, Minor=1`；唯一 Minor 是 1001 点审计并非形式化
  全域证明且压力路径物理选择仍未解决，已作为 ⚠️/❓ 明示，不影响只读实验有效性。
- ❌ **结论边界。** 没有在三条压力路径中选择正式实现，
  `h1bPressurePathSelectedAsFormal=false`、`authorizesRepair=false`、
  `formalModelPromotion=false`。未加载/仿真 SLX，未修改正式物性、SLX、MAT、
  PDF 或既有 H1a/H2/H2a 证据。Task 8、14000 s 稳态运行和论文 5.3
  稳态运行特性分析仍为 **RED/未完成**。
