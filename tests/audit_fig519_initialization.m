...
mappings = [ ...
    powerMapping("reactor", ...
        [model + "/reactor/Integrator6"; model + "/reactor/Integrator7"], ...
        [string(powerPaths.reactor.upstream_block); ...
         string(powerPaths.reactor.workspace_block)], ...
        "Recorded reactor power and temperature states feed the direct P_sw definition."); ...
    powerMapping("turbine", ...
        ...
        [string(powerPaths.turbine.block) + " output 4"; ...
         model + "/TAC/WT_sw"], ...
        "Recorded turbine-inlet thermal states and rotor speed map to the traced WT_sw component output."); ...
    powerMapping("compressor", ...
        ...
        [string(powerPaths.compressor.block) + " output 2"; ...
         model + "/TAC/Wc_sw"], ...
        "Recorded compressor-inlet thermal states and rotor speed map to the traced Wc_sw component output."); ...
    powerMapping("electrical_paper_eta", ...
        [model + "/TAC/rotor/N_rpm_Integrator"; ...
         model + "/IHX/IHX_region_2/T_c2_out_Integrator"; ...
         model + "/precooler/precooler_2/T_h2_out_Integrator"], ...
        [string(powerPaths.turbine.block) + " output 4"; model + "/TAC/WT_sw"; ...
         string(powerPaths.compressor.block) + " output 2"; model + "/TAC/Wc_sw"], ...
        "The paper-efficiency electrical definition is an offline function of the two traced shaft powers; no direct generator signal was found.")];
inventoryPaths = string({inventory.path});
mappingStateComplete = numel(mappings) == 4 && all(arrayfun(@(item) ...
    ~isempty(item.traced_state_paths) && ...
    all(ismember(string(item.traced_state_paths), inventoryPaths)), mappings));
mappingSignalComplete = numel(mappings) == 4 && all(arrayfun(@(item) ...
    ~isempty(item.traced_signal_paths) && ...
    all(strlength(string(item.traced_signal_paths)) > 0), mappings));
explanation = struct( ...
    "conclusion", ...
