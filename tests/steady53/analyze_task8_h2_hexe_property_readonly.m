function analysis = analyze_task8_h2_hexe_property_readonly(options)
%ANALYZE_TASK8_H2_HEXE_PROPERTY_READONLY Performs the Task 4 H2 audit.
%   The audit independently recomputes the active Virial intermediates and
%   calls the production property function only for final-output comparison.
%   It never loads or runs a Simulink model and never creates an H2 artifact.

root = fileparts(fileparts(fileparts(mfilename("fullpath"))));
config = defaultConfig(root);
if nargin > 0
    config = applyTestOnlyOptions(config, options);
end

validateExceptionPoint(config.exceptionT_K, config.exceptionP_Pa);
inputMat = requireAbsolutePath(config.inputMat, "inputMat");
modelPath = requireAbsolutePath(config.modelPath, "modelPath");
propertyPath = requireAbsolutePath(config.propertyPath, "propertyPath");
paperPdfPath = requireAbsolutePath(config.paperPdfPath, "paperPdfPath");
validateExpectedHash(config.expectedInputSha256, "expectedInputSha256");
validateExpectedHash(config.expectedModelSha256, "expectedModelSha256");
validateExpectedHash(config.expectedPropertySha256, ...
    "expectedPropertySha256");
validateExpectedHash(config.expectedPaperPdfSha256, ...
    "expectedPaperPdfSha256");

protectedBefore = protectedHashes(root);
inputMatSha256 = requireMatchingHash(inputMat, ...
    config.expectedInputSha256, "steady53:H2InputHashMismatch", "Input MAT");
modelSha256 = requireMatchingHash(modelPath, ...
    config.expectedModelSha256, "steady53:H2ModelHashMismatch", "Formal model");
propertySha256 = requireMatchingHash(propertyPath, ...
    config.expectedPropertySha256, "steady53:H2PropertyHashMismatch", ...
    "He-Xe property source");
paperPdfSha256 = requireMatchingHash(paperPdfPath, ...
    config.expectedPaperPdfSha256, "steady53:H2PaperPdfHashMismatch", ...
    "Thesis PDF");
archivePeeledCommit = requireArchivePeeledCommit(root, config.archiveTag, ...
    config.expectedArchivePeeledCommit);

% Task 1 intentionally allows this one, named-variable MAT read only.
payload = load(inputMat, "result", "report", "spec");
validatePayload(payload);
propertySource = fileread(propertyPath);
if strlength(string(propertySource)) == 0
    error("steady53:H2InvalidPropertySource", ...
        "He-Xe property source is unexpectedly empty.");
end
paperInfo = dir(paperPdfPath);
if numel(paperInfo) ~= 1 || paperInfo.bytes <= 0
    error("steady53:H2InvalidPaperPdf", ...
        "Thesis PDF metadata is invalid.");
end

equationMap = buildEquationMap();
coefficients = evaluateVirialCoefficients(config.exceptionT_K);
coefficients.status = "computedInTask2";
coefficients.classification = "calculatedConsequenceOfActiveSource";
zeroT_K = fzero(@componentC111, ...
    [config.exceptionT_K - 0.1, config.exceptionT_K + 0.1]);
coefficients.C111ZeroT_K = zeroT_K;
coefficients.C111AtZero = componentC111(zeroT_K);
coefficients.C111ZeroOffset_K = config.exceptionT_K - zeroT_K;

analytic = evaluateAnalyticDerivatives(config.exceptionT_K, coefficients);
finiteDifference = evaluateFiniteDifferences(config.exceptionT_K, analytic);
derivatives = struct( ...
    "status", "computedInTask2", ...
    "classification", "calculatedConsequenceOfActiveSource", ...
    "analytic", analytic, ...
    "finiteDifference", finiteDifference);

densityRoots = evaluateDensityRoots(config.exceptionT_K, ...
    config.exceptionP_Pa, coefficients);
diagnostic = evaluateThermalDiagnostic(config.exceptionT_K, coefficients, ...
    analytic, densityRoots.productionNewton.clampedFinal);
derivatives.drhoHat_dT = diagnostic.drhoHat_dT;
resolvedPropertyPath = string(which("HeXe_property_simulink"));
if resolvedPropertyPath ~= propertyPath
    error("steady53:H2PropertyResolutionMismatch", ...
        "Production property function resolved to an unapproved source.");
end
[calledCpMass, calledGamma, calledRho] = HeXe_property_simulink( ...
    config.exceptionT_K, config.exceptionP_Pa);
called = struct("cpMass", calledCpMass, "gamma", calledGamma, ...
    "rho", calledRho, "cvMassDerived", calledCpMass/calledGamma);
tolerances = struct("cpMassAbs", 1e-10, "gammaAbs", 1e-13, ...
    "rhoAbs", 1e-13);
absoluteError = struct( ...
    "cpMass", abs(diagnostic.cpMass - called.cpMass), ...
    "gamma", abs(diagnostic.gamma - called.gamma), ...
    "rho", abs(diagnostic.rho - called.rho));
allWithinTolerance = absoluteError.cpMass <= tolerances.cpMassAbs && ...
    absoluteError.gamma <= tolerances.gammaAbs && ...
    absoluteError.rho <= tolerances.rhoAbs;
if ~allWithinTolerance
    error("steady53:H2ProductionParityMismatch", ...
        "Independent Task 2 recomputation does not match production output.");
end
production = struct( ...
    "called", called, ...
    "diagnostic", diagnostic, ...
    "parity", struct("absoluteError", absoluteError, ...
        "tolerances", tolerances, "allWithinTolerance", allWithinTolerance));
thermoIdentity = evaluateThermodynamicIdentity( ...
    config.exceptionT_K, config.exceptionP_Pa, coefficients, analytic, ...
    densityRoots.productionNewton.clampedFinal, production);
domainSweep = evaluateDomainSweep(config, zeroT_K);
originalLiterature = originalLiteratureAudit();
hypothesisVerdicts = evaluateHypothesisVerdicts(derivatives, densityRoots, ...
    production, thermoIdentity, domainSweep, originalLiterature);

protectedAfter = protectedHashes(root);
if ~isequaln(protectedAfter, protectedBefore)
    error("steady53:H2ProtectedAssetChanged", ...
        "A protected asset changed during the read-only H2 audit.");
end

analysis = struct();
analysis.inputs = struct( ...
    "runId", config.runId, ...
    "exceptionT_K", config.exceptionT_K, ...
    "exceptionP_Pa", config.exceptionP_Pa, ...
    "inputMat", inputMat);
analysis.sourceAudit = struct( ...
    "inputMatSha256", inputMatSha256, ...
    "modelSha256", modelSha256, ...
    "propertySha256", propertySha256, ...
    "paperPdfSha256", paperPdfSha256, ...
    "paperPdfBytes", double(paperInfo.bytes), ...
    "archiveTag", config.archiveTag, ...
    "archivePeeledCommit", archivePeeledCommit, ...
    "reviewedPdfPages", [33 34 35], ...
    "reviewedPrintedPages", [18 19 20], ...
    "equationMap", equationMap, ...
    "resolvedPropertyPath", resolvedPropertyPath, ...
    "originalLiterature", originalLiterature, ...
    "protectedAssetHashesBefore", protectedBefore, ...
    "protectedAssetHashesAfter", protectedAfter);
analysis.coefficients = coefficients;
analysis.derivatives = derivatives;
analysis.densityRoots = densityRoots;
analysis.production = production;
analysis.thermoIdentity = thermoIdentity;
analysis.domainSweep = domainSweep;
analysis.hypothesisVerdicts = hypothesisVerdicts;
end

function map = buildEquationMap()
paperEquation = ["2.7"; "2.8"; "2.9"; "2.10"; "2.11"; "2.12"; ...
    "2.13"; "2.14"; "2.15"; "2.16"; "2.17"];
pdfPage = [33; 33; 33; 33; 34; 34; 34; 34; 34; 34; 34];
printedPage = [18; 18; 18; 18; 19; 19; 19; 19; 19; 19; 19];
sourceLineStart = [86; 50; 76; 71; 72; 74; 81; 79; 185; 175; 180];
sourceLineEnd = [98; 50; 76; 71; 73; 75; 83; 80; 194; 177; 182];
diagnosticPath = ["densityRoots"; "coefficients.mixtureMolarMass"; ...
    "coefficients.B"; "coefficients.B11"; "coefficients.B22"; ...
    "coefficients.B12"; "coefficients.C"; "coefficients.C111_C222"; ...
    "production.diagnostic.cpMass"; "derivatives.drhoHat_dT"; ...
    "production.diagnostic.cvMolar"];
map = table(paperEquation, pdfPage, printedPage, sourceLineStart, ...
    sourceLineEnd, diagnosticPath);
end

function audit = originalLiteratureAudit()
claims = [ ...
    "Eq.(11) third-order Virial fit is stated for Ne/Ar/Kr"
    "The source states the third Virial coefficient for He can be neglected"
    "40 g/mol He-Xe at T>400 K and P<2 MPa has under 1 percent ideal-gas-property error"];
audit = struct( ...
    "status", "verifiedInTask4", ...
    "evidenceGrade", "✅", ...
    "authors", "Tournier/El-Genk/Gallo", ...
    "paperNumber", "AIAA 2006-4154", ...
    "doi", "10.2514/6.2006-4154", ...
    "fullTextUrl", "https://www.researchgate.net/publication/268572975_Best_Estimates_of_Binary_Gas_Mixtures_Properties_for_Closed_Brayton_Cycle_Space_Applications", ...
    "officialMetadataUrl", "https://isnps.unm.edu/publications/529/", ...
    "fullTextEvidenceGrade", "✅", ...
    "officialMetadataEvidenceGrade", "✅", ...
    "claims", claims, ...
    "claimEvidenceGrade", repmat("✅", size(claims)), ...
    "fullTextEvidence", [ ...
        "Author-uploaded full text lines corresponding to Eq.(11) state the fit for Ne/Ar/Kr"
        "The same paragraph states that the third Virial coefficient for He can be neglected"
        "The abstract states the under-1-percent result for 40 g/mol He-Xe above 400 K and below 2 MPa"], ...
    "officialMetadataRelationship", ...
        "related2008JournalRecordNotExactAIAAConferenceMetadata", ...
    "officialMetadataRecord", struct( ...
        "title", "Properties of Noble Gases and Binary Mixtures for Closed Brayton Cycle Applications", ...
        "authors", "Tournier, J.-M., and M. S. El-Genk", ...
        "journal", "Energy Conversion and Management", ...
        "volume", "49(3)", "pages", "469-492", "year", 2008), ...
    "currentThesisFormulaKeptSeparate", true);
end

function constants = hexeConstants()
constants = struct();
constants.R0 = 8.314;
constants.M_He = 4.0026e-3;
constants.M_Xe = 131.293e-3;
constants.x_He = 0.7172;
constants.x_Xe = 1.0 - constants.x_He;
constants.T_c_He = 5.19;
constants.T_c_Xe = 289.6;
constants.rho_c_He = 69.64;
constants.rho_c_Xe = 1099.7;
constants.T_c_12 = sqrt(constants.T_c_He * constants.T_c_Xe);
constants.v_He = constants.M_He / constants.rho_c_He;
constants.v_Xe = constants.M_Xe / constants.rho_c_Xe;
constants.v_12 = (1/8) * (constants.v_He^(1/3) + ...
    constants.v_Xe^(1/3))^3;
constants.M = constants.x_Xe*constants.M_Xe + ...
    constants.x_He*constants.M_He;
end

function values = evaluateVirialCoefficients(T)
k = hexeConstants();
T_Xe = T/k.T_c_Xe;
T_12 = T/k.T_c_12;
values = struct();
values.mixtureMolarMass = k.M;
values.B11 = (8.4 - 0.0018*T + 115/sqrt(T) - 835/T)*1e-6;
u_Xe = 102.732 - 0.01*T_Xe - 0.44/T_Xe^1.22;
values.B22 = k.v_Xe*(-102.6 + u_Xe*tanh(4.5*sqrt(T_Xe)));
u_12 = 102.732 - 0.001*T_12 - 0.44/T_12^1.22;
values.B12 = k.v_12*(-102.6 + u_12*tanh(4.5*sqrt(T_12)));
values.B = k.x_He^2*values.B11 + ...
    2*k.x_He*k.x_Xe*values.B12 + k.x_Xe^2*values.B22;
values.C111 = componentC(T, k.v_He, k.T_c_He);
values.C222 = componentC(T, k.v_Xe, k.T_c_Xe);
values.C112 = signedRealCubeRoot(values.C111^2*values.C222);
values.C122 = signedRealCubeRoot(values.C111*values.C222^2);
values.C = k.x_He^3*values.C111 + ...
    3*k.x_He^2*k.x_Xe*values.C112 + ...
    3*k.x_He*k.x_Xe^2*values.C122 + k.x_Xe^3*values.C222;
values.C111_C222 = struct("C111", values.C111, "C222", values.C222);
end

function value = componentC111(T)
k = hexeConstants();
value = componentC(T, k.v_He, k.T_c_He);
end

function value = componentC(T, characteristicVolume, criticalTemperature)
theta = T/criticalTemperature;
value = characteristicVolume^2*(0.0757 + ...
    (-0.0862 - 3.6e-5*theta + 0.0237/theta^0.059)*tanh(0.84*theta));
end

function value = signedRealCubeRoot(argument)
value = sign(argument)*abs(argument)^(1/3);
end

function d = evaluateAnalyticDerivatives(T, values)
k = hexeConstants();
T_Xe = T/k.T_c_Xe;
T_12 = T/k.T_c_12;
d = struct();
d.dB11_dT = (-0.0018 - 57.5/T^(3/2) + 835/T^2)*1e-6;
d.d2B11_dT2 = (86.25/T^2.5 - 1670/T^3)*1e-6;
[d.dB22_dT, d.d2B22_dT2] = secondVirialDerivatives( ...
    T_Xe, k.T_c_Xe, k.v_Xe, 0.01);
[d.dB12_dT, d.d2B12_dT2] = secondVirialDerivatives( ...
    T_12, k.T_c_12, k.v_12, 0.001);
d.dB_dT = k.x_He^2*d.dB11_dT + ...
    2*k.x_He*k.x_Xe*d.dB12_dT + k.x_Xe^2*d.dB22_dT;
d.d2B_dT2 = k.x_He^2*d.d2B11_dT2 + ...
    2*k.x_He*k.x_Xe*d.d2B12_dT2 + k.x_Xe^2*d.d2B22_dT2;

[d.dC111_dT, d.d2C111_dT2] = thirdVirialDerivatives( ...
    T, k.T_c_He, k.v_He);
[d.dC222_dT, d.d2C222_dT2] = thirdVirialDerivatives( ...
    T, k.T_c_Xe, k.v_Xe);
a = d.dC111_dT/values.C111;
b = d.dC222_dT/values.C222;
d.dC112_dT = values.C112*((2/3)*a + (1/3)*b);
d.dC122_dT = values.C122*((1/3)*a + (2/3)*b);
aPrime = d.d2C111_dT2/values.C111 - a^2;
bPrime = d.d2C222_dT2/values.C222 - b^2;
d.d2C112_dT2 = values.C112*((2/3)*aPrime + (1/3)*bPrime + ...
    ((2/3)*a + (1/3)*b)^2);
d.d2C122_dT2 = values.C122*((1/3)*aPrime + (2/3)*bPrime + ...
    ((1/3)*a + (2/3)*b)^2);
d.dC_dT = k.x_He^3*d.dC111_dT + ...
    3*k.x_He^2*k.x_Xe*d.dC112_dT + ...
    3*k.x_He*k.x_Xe^2*d.dC122_dT + k.x_Xe^3*d.dC222_dT;
d.d2C_dT2 = k.x_He^3*d.d2C111_dT2 + ...
    3*k.x_He^2*k.x_Xe*d.d2C112_dT2 + ...
    3*k.x_He*k.x_Xe^2*d.d2C122_dT2 + k.x_Xe^3*d.d2C222_dT2;
end

function [first, second] = secondVirialDerivatives(theta, criticalT, v, slope)
u = 102.732 - slope*theta - 0.44/theta^1.22;
t = tanh(4.5*sqrt(theta));
s2 = 1 - t^2;
du = -slope + 0.5368/theta^2.22;
dt = 2.25/sqrt(theta)*s2;
d2u = -1.191696/theta^3.22;
d2t = -1.125/theta^1.5*s2 - 10.125/theta*s2*t;
first = v*(du*t + u*dt)/criticalT;
second = v*(d2u*t + 2*du*dt + u*d2t)/criticalT^2;
end

function [first, second] = thirdVirialDerivatives(T, criticalT, v)
theta = T/criticalT;
t = tanh(0.84*theta);
s2 = 1 - t^2;
u = -0.0862 - 3.6e-5*theta + 0.0237/theta^0.059;
du = -3.6e-5 - 0.0013983/theta^1.059;
dt = 0.84*s2;
d2u = 0.0014808/theta^2.059;
d2t = -1.4112*s2*t;
first = v^2*(du*t + u*dt)/criticalT;
second = v^2*(d2u*t + 2*du*dt + u*d2t)/criticalT^2;
end

function fd = evaluateFiniteDifferences(T, analytic)
names = ["B11" "B22" "B12" "B" ...
    "C111" "C222" "C112" "C122" "C"];
% B'' needs a larger window to avoid subtractive cancellation, while C is
% evaluated close to the C111 zero and needs a smaller zero-not-crossing window.
% All estimates remain ordinary centered differences of coefficient values.
steps = [1.6; 0.8; 0.4; 1.6e-3; 8e-4; 4e-4; 2e-4; 1e-4];
BSecondWindow = 1:3;
BFirstWindow = 4:6;
CWindow = 6:8;
first = struct();
second = struct();
for name = names
    first.(name) = zeros(numel(steps), 1);
    second.(name) = zeros(numel(steps), 1);
end
center = evaluateVirialCoefficients(T);
for index = 1:numel(steps)
    h = steps(index);
    below = evaluateVirialCoefficients(T - h);
    above = evaluateVirialCoefficients(T + h);
    for name = names
        first.(name)(index) = (above.(name) - below.(name))/(2*h);
        second.(name)(index) = ...
            (above.(name) - 2*center.(name) + below.(name))/h^2;
    end
end
richardson = struct( ...
    "dB_dT", richardsonEvidence(first.B, steps, BFirstWindow), ...
    "d2B_dT2", richardsonEvidence(second.B, steps, BSecondWindow), ...
    "dC_dT", richardsonEvidence(first.C, steps, CWindow), ...
    "d2C_dT2", richardsonEvidence(second.C, steps, CWindow));
convergence = struct( ...
    "BFirst", convergenceEvidence(first.B, analytic.dB_dT, 1e-7, BFirstWindow), ...
    "BSecond", convergenceEvidence(second.B, analytic.d2B_dT2, 1e-5, BSecondWindow), ...
    "CFirst", convergenceEvidence(first.C, analytic.dC_dT, 2e-4, CWindow), ...
    "CSecond", convergenceEvidence(second.C, analytic.d2C_dT2, 2e-3, CWindow));
fd = struct("stepSizes_K", steps, "first", first, "second", second, ...
    "richardson", richardson, "convergence", convergence, ...
    "method", "independentCenteredEvaluationOfCoefficientFunctions");
end

function evidence = richardsonEvidence(estimates, steps, indices)
selected = estimates(indices);
selectedSteps = steps(indices);
evidence = struct( ...
    "stepIndices", indices, ...
    "stepSizes_K", selectedSteps, ...
    "estimates", selected, ...
    "extrapolated", (4*selected(end) - selected(end - 1))/3);
end

function evidence = convergenceEvidence(estimates, analytic, tolerance, indices)
selected = estimates(indices);
successiveDelta = abs(diff(selected));
ratio = successiveDelta(1)/successiveDelta(2);
relativeError = abs(selected(end) - analytic)/max(abs(analytic), realmin);
evidence = struct( ...
    "stepIndices", indices, ...
    "successiveDelta", successiveDelta, ...
    "observedRefinementRatio", ratio, ...
    "finalRelativeError", relativeError, ...
    "requiredRelativeError", tolerance, ...
    "success", isfinite(relativeError) && relativeError < tolerance);
end

function density = evaluateDensityRoots(T, P_Pa, values)
k = hexeConstants();
P_RT = P_Pa/(k.R0*T);
polynomialCoefficients = [values.C values.B 1 -P_RT];
allRoots = roots(polynomialCoefficients);
realTolerance = 1e-10;
isReal = abs(imag(allRoots)) <= realTolerance.*max(1, abs(allRoots));
realRoots = real(allRoots(isReal));
polynomialResiduals = polyval(polynomialCoefficients, allRoots);
eosPressureResiduals_Pa = k.R0*T*polynomialResiduals;
residualScales = abs(polynomialCoefficients(1))*abs(allRoots).^3 + ...
    abs(polynomialCoefficients(2))*abs(allRoots).^2 + ...
    abs(polynomialCoefficients(3))*abs(allRoots) + ...
    abs(polynomialCoefficients(4));
normalizedPolynomialResiduals = abs(polynomialResiduals)./residualScales;
template = struct("root", 0, "polynomialResidual", 0, ...
    "eosPressureResidual_Pa", 0, "dPdrho_Pa_m3_per_mol", 0, ...
    "stabilitySign", "zero");
realRootDiagnostics = repmat(template, numel(realRoots), 1);
for index = 1:numel(realRoots)
    rootValue = realRoots(index);
    realRootDiagnostics(index).root = rootValue;
    realRootDiagnostics(index).polynomialResidual = ...
        polyval(polynomialCoefficients, rootValue);
    realRootDiagnostics(index).eosPressureResidual_Pa = ...
        k.R0*T*realRootDiagnostics(index).polynomialResidual;
    derivative = k.R0*T*(1 + 2*values.B*rootValue + ...
        3*values.C*rootValue^2);
    realRootDiagnostics(index).dPdrho_Pa_m3_per_mol = derivative;
    realRootDiagnostics(index).stabilitySign = signLabel(derivative);
end

rhoHat = P_RT;
initialGuess = rhoHat;
converged = false;
lastDelta = NaN;
for iteration = 1:30
    f = polyval(polynomialCoefficients, rhoHat);
    df = 3*values.C*rhoHat^2 + 2*values.B*rhoHat + 1;
    lastDelta = f/df;
    rhoHat = rhoHat - lastDelta;
    if abs(lastDelta) < 1e-14
        converged = true;
        break
    end
end
rawFinal = rhoHat;
clampFloor = 0.9*P_RT;
clampedFinal = max(rawFinal, clampFloor);
productionNewton = struct( ...
    "initialGuess", initialGuess, ...
    "maximumIterations", 30, ...
    "deltaTolerance", 1e-14, ...
    "iterations", iteration, ...
    "converged", converged, ...
    "lastDelta", lastDelta, ...
    "rawFinal", rawFinal, ...
    "rawPolynomialResidual", polyval(polynomialCoefficients, rawFinal), ...
    "rawEosPressureResidual_Pa", ...
        k.R0*T*polyval(polynomialCoefficients, rawFinal), ...
    "clampFloor", clampFloor, ...
    "clampedFinal", clampedFinal, ...
    "clampedPolynomialResidual", ...
        polyval(polynomialCoefficients, clampedFinal), ...
    "clampChanged", clampedFinal ~= rawFinal);
density = struct( ...
    "status", "computedInTask2", ...
    "P_RT", P_RT, ...
    "polynomialCoefficients", polynomialCoefficients, ...
    "allRoots", allRoots, ...
    "realTolerance", realTolerance, ...
    "isReal", isReal, ...
    "realRoots", realRoots, ...
    "polynomialResiduals", polynomialResiduals, ...
    "polynomialResidualScales", residualScales, ...
    "normalizedPolynomialResiduals", normalizedPolynomialResiduals, ...
    "eosPressureResiduals_Pa", eosPressureResiduals_Pa, ...
    "realRootDiagnostics", realRootDiagnostics, ...
    "productionNewton", productionNewton);
end

function label = signLabel(value)
if value > 0
    label = "positive";
elseif value < 0
    label = "negative";
else
    label = "zero";
end
end

function diagnostic = evaluateThermalDiagnostic(T, values, d, rhoHat)
k = hexeConstants();
drhoNumerator = (rhoHat + values.B*rhoHat^2 + values.C*rhoHat^3)/T + ...
    d.dB_dT*rhoHat^2 + d.dC_dT*rhoHat^3;
drhoDenominator = 1 + 2*values.B*rhoHat + 3*values.C*rhoHat^2;
drhoHat_dT = -drhoNumerator/drhoDenominator;
cvMolar = 1.5*k.R0 - rhoHat*k.R0*T*( ...
    2*d.dB_dT + T*d.d2B_dT2 + ...
    rhoHat*(d.dC_dT + 0.5*T*d.d2C_dT2));
B1 = values.B - T*d.dB_dT;
B2 = B1 - T^2*d.d2B_dT2;
C1 = 2*values.C - T*d.dC_dT;
C2 = values.C - 0.5*T^2*d.d2C_dT2;
cpMolar = 2.5*k.R0 + rhoHat*k.R0*(B2 + rhoHat*C2) + ...
    k.R0*T*(B1 + rhoHat*C1)*drhoHat_dT;
diagnostic = struct( ...
    "rhoHat", rhoHat, ...
    "drhoHat_dT", drhoHat_dT, ...
    "cpMolar", cpMolar, ...
    "cvMolar", cvMolar, ...
    "cpMass", cpMolar/k.M, ...
    "cvMass", cvMolar/k.M, ...
    "gamma", cpMolar/cvMolar, ...
    "rho", rhoHat*k.M);
end

function thermo = evaluateThermodynamicIdentity( ...
        T, P_Pa, values, d, rhoHat, production)
k = hexeConstants();
cpMolar = production.diagnostic.cpMolar;
cvMolar = production.diagnostic.cvMolar;
drhoHat_dT = production.diagnostic.drhoHat_dT;

enthalpyFiniteDifference = evaluateEnthalpyFiniteDifference( ...
    T, P_Pa, cpMolar);
dP_dT_atRho = k.R0*(rhoHat + values.B*rhoHat^2 + ...
    values.C*rhoHat^3) + k.R0*T*( ...
    d.dB_dT*rhoHat^2 + d.dC_dT*rhoHat^3);
lhsCpMinusCv = cpMolar - cvMolar;
rhsCpMinusCv = T*dP_dT_atRho*(-drhoHat_dT)/rhoHat^2;
identityRelativeError = abs(lhsCpMinusCv - rhsCpMinusCv) / ...
    max([abs(lhsCpMinusCv), abs(rhsCpMinusCv), realmin]);

analyticGamma = cpMolar/cvMolar;
gammaAbsoluteError = abs(analyticGamma - production.called.gamma);
consistencyTolerances = struct( ...
    "cpEnthalpyRelative", 2e-5, ...
    "cpMinusCvRelative", 1e-12, ...
    "gammaAbsolute", 1e-13);
formulaConsistency = struct( ...
    "cpVsEnthalpy", enthalpyFiniteDifference.finalRelativeError <= ...
        consistencyTolerances.cpEnthalpyRelative, ...
    "cpMinusCvIdentity", identityRelativeError <= ...
        consistencyTolerances.cpMinusCvRelative, ...
    "gammaParity", gammaAbsoluteError <= ...
        consistencyTolerances.gammaAbsolute);
formulaConsistency.allSatisfied = formulaConsistency.cpVsEnthalpy && ...
    formulaConsistency.cpMinusCvIdentity && ...
    formulaConsistency.gammaParity;
formulaConsistency.tolerances = consistencyTolerances;
if ~formulaConsistency.allSatisfied
    error("steady53:H2ThermodynamicIdentityMismatch", ...
        "Task 3 thermodynamic identities do not agree within tolerance.");
end

physicalDomain = struct( ...
    "cpPositive", cpMolar > 0, ...
    "cvPositive", cvMolar > 0, ...
    "gammaGreaterThanOne", analyticGamma > 1);
physicalDomain.allSatisfied = physicalDomain.cpPositive && ...
    physicalDomain.cvPositive && physicalDomain.gammaGreaterThanOne;

thermo = struct( ...
    "status", "computedInTask3", ...
    "classification", "calculatedConsequenceOfActiveSource", ...
    "eq2_15", struct("analyticCpMolar", cpMolar), ...
    "enthalpyFiniteDifference", enthalpyFiniteDifference, ...
    "eq2_17", struct("analyticCvMolar", cvMolar), ...
    "eosIdentity", struct( ...
        "rhoHat", rhoHat, ...
        "dP_dT_atRho_Pa_K", dP_dT_atRho, ...
        "minusDrhoHat_dT_atP_mol_m3_K", -drhoHat_dT, ...
        "lhsCpMinusCv_Molar", lhsCpMinusCv, ...
        "rhsMolar", rhsCpMinusCv, ...
        "absoluteError", abs(lhsCpMinusCv - rhsCpMinusCv), ...
        "relativeError", identityRelativeError), ...
    "gamma", struct("analytic", analyticGamma, ...
        "production", production.called.gamma, ...
        "absoluteError", gammaAbsoluteError), ...
    "formulaConsistency", formulaConsistency, ...
    "physicalDomain", physicalDomain, ...
    "contributions", evaluateHeatCapacityContributions( ...
        T, values, d, rhoHat, drhoHat_dT));
end

function evidence = evaluateEnthalpyFiniteDifference(T, P_Pa, analyticCp)
steps = [1.6e-3; 8e-4; 4e-4; 2e-4];
estimates = zeros(size(steps));
for index = 1:numel(steps)
    h = steps(index);
    below = evaluateMolarEnthalpy(T - h, P_Pa);
    above = evaluateMolarEnthalpy(T + h, P_Pa);
    estimates(index) = (above.totalMolar - below.totalMolar)/(2*h);
end
richardson = (4*estimates(end) - estimates(end - 1))/3;
relativeErrors = abs(estimates - analyticCp) / ...
    max(abs(analyticCp), realmin);
evidence = struct( ...
    "method", "fixedPressureCenteredDifferenceOfIdealPlusResidualEnthalpy", ...
    "stepSizes_K", steps, ...
    "cpMolarEstimates", estimates, ...
    "richardsonEstimate", richardson, ...
    "relativeErrors", relativeErrors, ...
    "finalRelativeError", abs(richardson - analyticCp) / ...
        max(abs(analyticCp), realmin));
end

function enthalpy = evaluateMolarEnthalpy(T, P_Pa)
k = hexeConstants();
values = evaluateVirialCoefficients(T);
d = evaluateAnalyticDerivatives(T, values);
density = evaluateDensityRoots(T, P_Pa, values);
rhoHat = density.productionNewton.clampedFinal;
B1 = values.B - T*d.dB_dT;
C1 = 2*values.C - T*d.dC_dT;
idealMolar = 2.5*k.R0*T;
residualBMolar = k.R0*T*rhoHat*B1;
residualCMolar = 0.5*k.R0*T*rhoHat^2*C1;
enthalpy = struct( ...
    "idealMolar", idealMolar, ...
    "residualBMolar", residualBMolar, ...
    "residualCMolar", residualCMolar, ...
    "totalMolar", idealMolar + residualBMolar + residualCMolar, ...
    "rhoHat", rhoHat, ...
    "densityClampChanged", density.productionNewton.clampChanged);
end

function contributions = evaluateHeatCapacityContributions( ...
        T, values, d, rhoHat, drhoHat_dT)
k = hexeConstants();
B1 = values.B - T*d.dB_dT;
B2 = B1 - T^2*d.d2B_dT2;
C1 = 2*values.C - T*d.dC_dT;
C2 = values.C - 0.5*T^2*d.d2C_dT2;

cpIdeal = 2.5*k.R0;
cpBExplicit = rhoHat*k.R0*B2;
cpCExplicit = rhoHat^2*k.R0*C2;
cpDensityB = k.R0*T*B1*drhoHat_dT;
cpDensityC = k.R0*T*rhoHat*C1*drhoHat_dT;
cpDensityTotal = cpDensityB + cpDensityC;
cpTotal = cpIdeal + cpBExplicit + cpCExplicit + cpDensityTotal;

cvIdeal = 1.5*k.R0;
cvB = -rhoHat*k.R0*T*(2*d.dB_dT + T*d.d2B_dT2);
cvC = -rhoHat^2*k.R0*T*(d.dC_dT + ...
    0.5*T*d.d2C_dT2);
cvTotal = cvIdeal + cvB + cvC;

names = ["C111"; "C222"; "C112"; "C122"];
mixtureWeight = [k.x_He^3; k.x_Xe^3; ...
    3*k.x_He^2*k.x_Xe; 3*k.x_He*k.x_Xe^2];
componentValue = [values.C111; values.C222; values.C112; values.C122];
componentFirst = [d.dC111_dT; d.dC222_dT; ...
    d.dC112_dT; d.dC122_dT];
componentSecond = [d.d2C111_dT2; d.d2C222_dT2; ...
    d.d2C112_dT2; d.d2C122_dT2];
valueContribution = mixtureWeight.*componentValue;
firstDerivativeContribution = mixtureWeight.*componentFirst;
secondDerivativeContribution = mixtureWeight.*componentSecond;
cpExplicitMolar = rhoHat^2*k.R0*(valueContribution - ...
    0.5*T^2*secondDerivativeContribution);
cpDensityDerivativeMolar = k.R0*T*rhoHat*( ...
    2*valueContribution - T*firstDerivativeContribution)*drhoHat_dT;
cvMolar = -rhoHat^2*k.R0*T*(firstDerivativeContribution + ...
    0.5*T*secondDerivativeContribution);
thirdVirialComponents = table(names, mixtureWeight, valueContribution, ...
    firstDerivativeContribution, secondDerivativeContribution, ...
    cpExplicitMolar, cpDensityDerivativeMolar, cvMolar, ...
    'VariableNames', {'name', 'mixtureWeight', 'valueContribution', ...
    'firstDerivativeContribution', 'secondDerivativeContribution', ...
    'cpExplicitMolar', 'cpDensityDerivativeMolar', 'cvMolar'});

contributions = struct( ...
    "cpMolar", struct( ...
        "ideal", cpIdeal, ...
        "BExplicit", cpBExplicit, ...
        "CExplicit", cpCExplicit, ...
        "densityDerivativeB", cpDensityB, ...
        "densityDerivativeC", cpDensityC, ...
        "densityDerivativeTotal", cpDensityTotal, ...
        "total", cpTotal), ...
    "cvMolar", struct( ...
        "ideal", cvIdeal, "B", cvB, "C", cvC, "total", cvTotal), ...
    "thirdVirialComponents", thirdVirialComponents);
end

function sweep = evaluateDomainSweep(config, zeroT_K)
persistent cachedSweep cachedKey
cacheKey = [zeroT_K config.exceptionT_K config.exceptionP_Pa];
if ~isempty(cachedKey) && isequal(cachedKey, cacheKey)
    sweep = cachedSweep;
    return
end
quantities = ["cp=0" "cv=0" "gamma=1" "dP/drho=0"];
offsets_K = [1e-8; 3e-8; 1e-7; 3e-7; 1e-6; 3e-6; ...
    1e-5; 3e-5; 1e-4; 3e-4; 1e-3; 3e-3; 1e-2; 3e-2; 0.1];

fixedCoarse = linspace(zeroT_K - 0.1, zeroT_K + 0.1, 40)';
fixedAdaptive = unique([fixedCoarse; zeroT_K - offsets_K; ...
    zeroT_K + offsets_K]);
fixedPressure = evaluateSweepPath("fixedPressure", fixedCoarse, ...
    fixedAdaptive, zeroT_K, config, quantities);

T1_K = 1515.109678670083;
P1_Pa = 1538809.802594816;
expansionRatio = 2.2812178550028612;
Tlow_K = T1_K/expansionRatio;
P2_Pa = 674556.267925093;
zeroLambda = (zeroT_K - T1_K)/(Tlow_K - T1_K);
lambdaOffsets = offsets_K/abs(Tlow_K - T1_K);
pathCoarse = linspace(0, 1, 41)';
pathAdaptive = unique([pathCoarse; zeroLambda - lambdaOffsets; ...
    zeroLambda + lambdaOffsets]);
pathAdaptive = pathAdaptive(pathAdaptive >= 0 & pathAdaptive <= 1);
h1aLowEndPath = evaluateSweepPath("h1aLowEndPath", pathCoarse, ...
    pathAdaptive, zeroT_K, config, quantities);
h1aLowEndPath.T1_K = T1_K;
h1aLowEndPath.P1_Pa = P1_Pa;
h1aLowEndPath.expansionRatio = expansionRatio;
h1aLowEndPath.Tlow_K = Tlow_K;
h1aLowEndPath.P2_Pa = P2_Pa;

leftCp = zeros(size(offsets_K));
rightCp = zeros(size(offsets_K));
for index = 1:numel(offsets_K)
    leftState = evaluateSweepState(zeroT_K - offsets_K(index), ...
        config.exceptionP_Pa);
    rightState = evaluateSweepState(zeroT_K + offsets_K(index), ...
        config.exceptionP_Pa);
    leftCp(index) = leftState.cpMolar;
    rightCp(index) = rightState.cpMolar;
end
smallMagnitude = max(abs([leftCp(1) rightCp(1)]));
largeMagnitude = max(abs([leftCp(end) rightCp(end)]));
derivativeIsNonContinuous = isfinite(smallMagnitude) && ...
    isfinite(largeMagnitude) && smallMagnitude > 10*largeMagnitude;
if ~derivativeIsNonContinuous
    error("steady53:H2SingularityEvidenceIncomplete", ...
        "C111 mixed-term derivative discontinuity was not resolved.");
end
singularity = struct( ...
    "classification", ...
        "C111ZeroAndMixedThirdVirialDerivativeDiscontinuity", ...
    "rootT_K", zeroT_K, ...
    "C111Residual", componentC111(zeroT_K), ...
    "h1aLambda", zeroLambda, ...
    "h1aPressure_Pa", P1_Pa + zeroLambda*(P2_Pa - P1_Pa), ...
    "neighborhoodOffsets_K", offsets_K, ...
    "leftCpMolar", leftCp, ...
    "rightCpMolar", rightCp, ...
    "leftClassification", arrayfun(@signLabel, leftCp), ...
    "rightClassification", arrayfun(@signLabel, rightCp), ...
    "derivativeIsNonContinuous", derivativeIsNonContinuous, ...
    "boundaryTreatment", ...
        "isolatedFromRootBracketsAndRecordedWithOneSidedAdaptiveStates");

sweep = struct( ...
    "status", "completedTask4", ...
    "evidenceGrade", "✅", ...
    "quantitiesSearched", quantities, ...
    "method", struct( ...
        "coarseGridOnlyFindsBrackets", true, ...
        "rootSolver", "fzero", ...
        "adaptiveLevels", numel(offsets_K), ...
        "crossSingularityBracketsExcluded", true, ...
        "visualJudgmentUsed", false), ...
    "fixedPressure", fixedPressure, ...
    "h1aLowEndPath", h1aLowEndPath, ...
    "C111DerivativeDiscontinuity", singularity);
cachedKey = cacheKey;
cachedSweep = sweep;
end

function scan = evaluateSweepPath(pathName, coarseCoordinate, ...
        adaptiveCoordinate, zeroT_K, config, quantities)
adaptiveCoordinate = sort(unique(adaptiveCoordinate(:)));
states = repmat(emptySweepState(), numel(adaptiveCoordinate), 1);
for index = 1:numel(adaptiveCoordinate)
    [T_K, P_Pa] = coordinateToStatePoint( ...
        pathName, adaptiveCoordinate(index), config);
    if abs(T_K - zeroT_K) <= 1e-12
        states(index).T_K = T_K;
        states(index).P_Pa = P_Pa;
        continue
    end
    states(index) = evaluateSweepState(T_K, P_Pa);
end

boundaries = emptyBoundaryTable();
for quantity = quantities
    values = arrayfun(@(state) sweepMetric(state, quantity), states);
    tolerance = metricResidualTolerance(quantity);
    for index = 1:(numel(adaptiveCoordinate) - 1)
        leftCoordinate = adaptiveCoordinate(index);
        rightCoordinate = adaptiveCoordinate(index + 1);
        leftValue = values(index);
        rightValue = values(index + 1);
        leftT = states(index).T_K;
        rightT = states(index + 1).T_K;
        crossesSingularity = (leftT - zeroT_K)*(rightT - zeroT_K) < 0;
        if crossesSingularity || ~isfinite(leftValue) || ...
                ~isfinite(rightValue) || sign(leftValue) == sign(rightValue)
            continue
        end
        if leftValue == 0 || rightValue == 0
            continue
        end
        metricFunction = @(coordinate) sweepMetricAtCoordinate( ...
            pathName, coordinate, config, quantity);
        options = optimset("TolX", 1e-12, "Display", "off");
        [rootCoordinate, residual, exitFlag] = fzero(metricFunction, ...
            [leftCoordinate rightCoordinate], options);
        if quantity == "gamma=1" && isfinite(rootCoordinate)
            [candidateT, candidateP] = coordinateToStatePoint( ...
                pathName, rootCoordinate, config);
            candidateState = evaluateSweepState(candidateT, candidateP);
            if abs(candidateState.cvMolar) <= 1e-4 && ...
                    abs(residual) > tolerance
                % gamma has a pole at cv=0.  That sign change belongs to
                % the separately refined cv boundary, not gamma=1.
                continue
            end
        end
        if exitFlag <= 0 || ~isfinite(rootCoordinate) || ...
                ~isfinite(residual) || abs(residual) > tolerance
            error("steady53:H2BoundaryRefinementFailed", ...
                "Could not refine %s on %s within its residual contract.", ...
                quantity, pathName);
        end
        if any(boundaries.quantity == quantity & ...
                abs(boundaries.rootCoordinate - rootCoordinate) <= 1e-9)
            continue
        end
        [rootT_K, rootP_Pa] = coordinateToStatePoint( ...
            pathName, rootCoordinate, config);
        row = table(quantity, leftCoordinate, rightCoordinate, ...
            rootCoordinate, rootT_K, rootP_Pa, residual, tolerance, ...
            leftValue, rightValue, signLabel(leftValue), ...
            signLabel(rightValue), ...
            'VariableNames', boundaries.Properties.VariableNames);
        boundaries = [boundaries; row]; %#ok<AGROW>
    end
end

counts = zeros(numel(quantities), 1);
for index = 1:numel(quantities)
    counts(index) = sum(boundaries.quantity == quantities(index));
end
boundaryCountByQuantity = table(quantities(:), counts, ...
    'VariableNames', {'quantity', 'count'});
if ~isequal(counts, [1; 1; 0; 0])
    error("steady53:H2UnexpectedBoundaryCount", ...
        "Task 4 did not resolve the approved cp/cv/gamma/dPdrho boundary counts.");
end
cvRow = boundaries.quantity == "cv=0";
gammaRow = boundaries.quantity == "gamma=1";
cvBoundary = boundaries(cvRow, :);
poleDelta = min(cvBoundary.rootCoordinate - cvBoundary.bracketLeft, ...
    cvBoundary.bracketRight - cvBoundary.rootCoordinate)*1e-3;
leftPoleCoordinate = cvBoundary.rootCoordinate - poleDelta;
rightPoleCoordinate = cvBoundary.rootCoordinate + poleDelta;
[leftT, leftP] = coordinateToStatePoint(pathName, ...
    leftPoleCoordinate, config);
[rightT, rightP] = coordinateToStatePoint(pathName, ...
    rightPoleCoordinate, config);
leftGamma = evaluateSweepState(leftT, leftP).gammaMinusOne + 1;
rightGamma = evaluateSweepState(rightT, rightP).gammaMinusOne + 1;
gammaPoleAtCvZero = struct( ...
    "classification", "gammaPoleAtCvZeroNotGammaEqualsOneRoot", ...
    "cvBoundaryCount", sum(cvRow), ...
    "gammaEqualsOneBoundaryCount", sum(gammaRow), ...
    "cvRootCoordinate", cvBoundary.rootCoordinate, ...
    "cvRootT_K", cvBoundary.T_K, ...
    "cvRootP_Pa", cvBoundary.P_Pa, ...
    "leftCoordinate", leftPoleCoordinate, ...
    "rightCoordinate", rightPoleCoordinate, ...
    "leftGamma", leftGamma, ...
    "rightGamma", rightGamma, ...
    "oppositeSignedOneSidedGamma", sign(leftGamma) ~= sign(rightGamma), ...
    "isGammaEqualsOneRoot", false);
if gammaPoleAtCvZero.cvBoundaryCount ~= 1 || ...
        gammaPoleAtCvZero.gammaEqualsOneBoundaryCount ~= 0 || ...
        ~gammaPoleAtCvZero.oppositeSignedOneSidedGamma
    error("steady53:H2GammaPoleClassificationFailed", ...
        "The cv=0 gamma pole was not isolated from gamma=1 roots.");
end
scan = struct( ...
    "pathName", pathName, ...
    "quantitiesSearched", quantities, ...
    "coarseCoordinate", coarseCoordinate(:), ...
    "adaptiveCoordinate", adaptiveCoordinate, ...
    "adaptiveState", struct2table(states), ...
    "boundaries", boundaries, ...
    "boundaryCountByQuantity", boundaryCountByQuantity, ...
    "gammaPoleAtCvZero", gammaPoleAtCvZero);
if pathName == "fixedPressure"
    scan.coordinateName = "T_K";
    scan.pressure_Pa = config.exceptionP_Pa;
    scan.temperatureRange_K = [min(adaptiveCoordinate) max(adaptiveCoordinate)];
else
    scan.coordinateName = "lambda";
    scan.lambdaRange = [min(adaptiveCoordinate) max(adaptiveCoordinate)];
end
end

function state = emptySweepState()
state = struct("T_K", NaN, "P_Pa", NaN, "cpMolar", NaN, ...
    "cvMolar", NaN, "gammaMinusOne", NaN, ...
    "dPdrho_Pa_m3_per_mol", NaN, "rhoHat", NaN, ...
    "densityResidual_Pa", NaN, "stablePositiveRealRootCount", NaN);
end

function state = evaluateSweepState(T_K, P_Pa)
values = evaluateVirialCoefficients(T_K);
d = evaluateAnalyticDerivatives(T_K, values);
density = evaluateDensityRoots(T_K, P_Pa, values);
rhoHat = density.productionNewton.clampedFinal;
thermal = evaluateThermalDiagnostic(T_K, values, d, rhoHat);
k = hexeConstants();
dPdrho = k.R0*T_K*(1 + 2*values.B*rhoHat + ...
    3*values.C*rhoHat^2);
rootDiagnostics = density.realRootDiagnostics;
if isempty(rootDiagnostics)
    stablePositiveCount = 0;
else
    rootValues = [rootDiagnostics.root];
    rootSlopes = [rootDiagnostics.dPdrho_Pa_m3_per_mol];
    rootResiduals = abs([rootDiagnostics.eosPressureResidual_Pa]);
    stablePositiveCount = sum(rootValues > 0 & rootSlopes > 0 & ...
        rootResiduals <= 1e-6);
end
state = struct( ...
    "T_K", T_K, "P_Pa", P_Pa, ...
    "cpMolar", thermal.cpMolar, ...
    "cvMolar", thermal.cvMolar, ...
    "gammaMinusOne", thermal.gamma - 1, ...
    "dPdrho_Pa_m3_per_mol", dPdrho, ...
    "rhoHat", rhoHat, ...
    "densityResidual_Pa", density.productionNewton.rawEosPressureResidual_Pa, ...
    "stablePositiveRealRootCount", stablePositiveCount);
end

function value = sweepMetricAtCoordinate(pathName, coordinate, config, quantity)
[T_K, P_Pa] = coordinateToStatePoint(pathName, coordinate, config);
value = sweepMetric(evaluateSweepState(T_K, P_Pa), quantity);
end

function value = sweepMetric(state, quantity)
switch quantity
    case "cp=0"
        value = state.cpMolar;
    case "cv=0"
        value = state.cvMolar;
    case "gamma=1"
        value = state.gammaMinusOne;
    case "dP/drho=0"
        value = state.dPdrho_Pa_m3_per_mol;
    otherwise
        error("steady53:H2UnknownBoundary", ...
            "Unknown Task 4 boundary quantity '%s'.", quantity);
end
end

function tolerance = metricResidualTolerance(quantity)
switch quantity
    case {"cp=0", "cv=0"}
        tolerance = 1e-6;
    case "gamma=1"
        tolerance = 1e-10;
    case "dP/drho=0"
        tolerance = 1e-3;
end
end

function [T_K, P_Pa] = coordinateToStatePoint(pathName, coordinate, config)
if pathName == "fixedPressure"
    T_K = coordinate;
    P_Pa = config.exceptionP_Pa;
    return
end
T1_K = 1515.109678670083;
P1_Pa = 1538809.802594816;
Tlow_K = T1_K/2.2812178550028612;
P2_Pa = 674556.267925093;
T_K = T1_K + coordinate*(Tlow_K - T1_K);
P_Pa = P1_Pa + coordinate*(P2_Pa - P1_Pa);
end

function boundaries = emptyBoundaryTable()
boundaries = table(strings(0, 1), zeros(0, 1), zeros(0, 1), ...
    zeros(0, 1), zeros(0, 1), zeros(0, 1), zeros(0, 1), ...
    zeros(0, 1), zeros(0, 1), zeros(0, 1), strings(0, 1), ...
    strings(0, 1), 'VariableNames', { ...
    'quantity', 'bracketLeft', 'bracketRight', 'rootCoordinate', ...
    'T_K', 'P_Pa', 'residual', 'residualTolerance', 'leftValue', ...
    'rightValue', 'leftState', 'rightState'});
end

function verdicts = evaluateHypothesisVerdicts(derivatives, densityRoots, ...
        production, thermoIdentity, domainSweep, literature)
convergence = derivatives.finiteDifference.convergence;
derivativeChecksPass = convergence.BFirst.success && ...
    convergence.BSecond.success && convergence.CFirst.success && ...
    convergence.CSecond.success;
formulaConsistent = derivativeChecksPass && ...
    production.parity.allWithinTolerance && ...
    thermoIdentity.formulaConsistency.allSatisfied;

rootDiagnostics = densityRoots.realRootDiagnostics;
rootValues = [rootDiagnostics.root];
rootSlopes = [rootDiagnostics.dPdrho_Pa_m3_per_mol];
rootResiduals = abs([rootDiagnostics.eosPressureResidual_Pa]);
stablePositive = rootValues > 0 & rootSlopes > 0 & rootResiduals <= 1e-6;
uniqueStableRoot = sum(stablePositive) == 1;
if uniqueStableRoot
    selectedRoot = rootValues(stablePositive);
    productionMatchesUniqueRoot = abs( ...
        densityRoots.productionNewton.clampedFinal - selectedRoot) <= ...
        1e-10*max(1, abs(selectedRoot));
else
    productionMatchesUniqueRoot = false;
end
uniqueStableDensityRoot = uniqueStableRoot && productionMatchesUniqueRoot && ...
    ~densityRoots.productionNewton.clampChanged;
physicalDomainSatisfied = thermoIdentity.physicalDomain.allSatisfied;

implementationSupported = ~formulaConsistent;
densityRootSupported = formulaConsistent && ~uniqueStableDensityRoot;
correlationSupported = formulaConsistent && uniqueStableDensityRoot && ...
    ~physicalDomainSatisfied;
if implementationSupported || densityRootSupported || ~correlationSupported || ...
        domainSweep.status ~= "completedTask4" || ...
        literature.status ~= "verifiedInTask4"
    error("steady53:H2VerdictEvidenceIncomplete", ...
        "Task 4 evidence does not support the approved complete verdict set.");
end

verdicts = struct( ...
    "status", "completedTask4", ...
    "implementationError", struct( ...
        "hypothesis", ...
            "current transcription or analytic derivative implementation is wrong relative to thesis Eq.2.7-2.17", ...
        "supported", false, "evidenceGrade", "❌", ...
        "basis", ...
            "equation mapping, independent finite differences, production parity, enthalpy derivative and EOS identity all agree"), ...
    "densityRootError", struct( ...
        "hypothesis", "production selected the wrong density root", ...
        "supported", false, "evidenceGrade", "❌", ...
        "uniqueStablePositiveRealRoot", uniqueStableRoot, ...
        "productionMatchesUniqueRoot", productionMatchesUniqueRoot, ...
        "clampChanged", densityRoots.productionNewton.clampChanged), ...
    "directPaperCorrelationNonphysical", struct( ...
        "hypothesis", ...
            "direct implementation of the current thesis correlation is nonphysical at the exception point", ...
        "supported", true, "evidenceGrade", "✅", ...
        "formulaConsistent", formulaConsistent, ...
        "uniqueStableDensityRoot", uniqueStableDensityRoot, ...
        "physicalDomainSatisfied", physicalDomainSatisfied), ...
    "provenanceApplicability", struct( ...
        "status", ...
            "sourceFrameworkDiffersFromCurrentThesisFormulaUse", ...
        "evidenceGrade", "✅", ...
        "sourceSaysEq11FitFor", "Ne/Ar/Kr", ...
        "sourceSaysHeliumThirdVirial", "can be neglected", ...
        "sourceIdealGasRegion", "40 g/mol He-Xe; T>400 K; P<2 MPa; under 1 percent error", ...
        "keptSeparateFromCurrentThesisFormula", true), ...
    "authorizesRepair", false, ...
    "h1aS2Status", "BLOCKED_BY_PROPERTY_DOMAIN", ...
    "h2Status", "READ_ONLY_ROOT_CAUSE_COMPLETE", ...
    "task8Status", "RED_NOT_COMPLETED", ...
    "steady14000sStatus", "NOT_COMPLETED");
end

function config = defaultConfig(root)
runId = "run_1787582761047_bb4aa60600cc4d9e9cc15077c6f435d3";
config = struct();
config.testOnly = false;
config.runId = runId;
config.inputMat = string(fullfile(root, "tmp", "steady53", "task8", ...
    runId, "nominal_500_report.mat"));
config.expectedInputSha256 = ...
    "4ea018c7be06c5e577f107970dc2bf549924bf7a9a2989a89bcf1be76e98472b";
config.modelPath = string(fullfile(root, "final_steady_24a.slx"));
config.expectedModelSha256 = ...
    "5423af38d6bbfc7730529475a6c4d046ef1386ec56782ba465c87dfae82cbf5d";
config.propertyPath = string(fullfile(root, "HeXe_property_simulink.m"));
config.expectedPropertySha256 = ...
    "2490785cba7ae3d1f9bb4d4e52621f7b925945aab0f4f93e1a71b504783f5cf2";
config.paperPdfPath = string(fullfile( ...
    "/Users/ikunsredemptionmac/Downloads/不接入转子稳态模型", ...
    "空间锂冷堆He-Xe布雷顿循环发电系统优化设计与运行特性分析_徐驰.pdf"));
config.expectedPaperPdfSha256 = ...
    "983bfc23712221f30202a47875cbe34c9559edf79b9c332aa20931b6075e4e7a";
config.archiveTag = "archive/pre-restart-20260824";
config.expectedArchivePeeledCommit = ...
    "8f625c268c35a95c18a626305c1aa6a79ae2ace7";
config.exceptionT_K = 992.38742737169468;
config.exceptionP_Pa = 1007910.8613125964;
end

function config = applyTestOnlyOptions(config, options)
if ~isstruct(options) || ~isscalar(options) || ...
        ~isfield(options, "testOnly") || ~isequal(options.testOnly, true)
    error("steady53:H2InvalidOptions", ...
        "Non-default options require explicit testOnly=true.");
end
allowed = ["testOnly" "inputMat" "expectedInputSha256" "modelPath" ...
    "expectedModelSha256" "propertyPath" "expectedPropertySha256" ...
    "paperPdfPath" "expectedPaperPdfSha256" "archiveTag" ...
    "expectedArchivePeeledCommit" "exceptionT_K" "exceptionP_Pa"];
actual = string(fieldnames(options));
if any(~ismember(actual, allowed)) || ~all(ismember(allowed, actual))
    error("steady53:H2InvalidOptions", ...
        "Test-only options must state the complete override contract.");
end
for index = 1:numel(allowed)
    config.(allowed(index)) = options.(allowed(index));
end
end

function validateExceptionPoint(T_K, P_Pa)
if ~isscalar(T_K) || ~isreal(T_K) || ~isfinite(T_K) || ...
        ~isscalar(P_Pa) || ~isreal(P_Pa) || ~isfinite(P_Pa) || ...
        T_K ~= 992.38742737169468 || P_Pa ~= 1007910.8613125964
    error("steady53:H2ExceptionPointMismatch", ...
        "H2 is restricted to the approved (T,P) exception point.");
end
end

function pathValue = requireAbsolutePath(value, label)
if ~(isstring(value) || ischar(value)) || ...
        ~isscalar(string(value)) || ismissing(string(value)) || ...
        strlength(string(value)) == 0 || ~startsWith(string(value), filesep)
    error("steady53:H2InvalidOptions", "%s must be an absolute path.", label);
end
pathValue = string(value);
end

function validateExpectedHash(value, label)
if ~(isstring(value) || ischar(value)) || ...
        isempty(regexp(char(string(value)), '^[0-9a-fA-F]{64}$', 'once'))
    error("steady53:H2InvalidOptions", "%s must be a SHA-256 value.", label);
end
end

function actual = requireMatchingHash(filePath, expected, identifier, label)
actual = sha256File(filePath);
if actual ~= lower(string(expected))
    error(identifier, "%s hash mismatch for '%s'.", label, filePath);
end
end

function commit = requireArchivePeeledCommit(root, tagName, expectedCommit)
if ~(isstring(expectedCommit) || ischar(expectedCommit)) || ...
        isempty(regexp(char(string(expectedCommit)), ...
        '^[0-9a-fA-F]{40}$', 'once'))
    error("steady53:H2InvalidOptions", ...
        "expectedArchivePeeledCommit must be a Git SHA-1 commit ID.");
end
if ~(isstring(tagName) || ischar(tagName)) || ...
        ~isscalar(string(tagName)) || strlength(string(tagName)) == 0
    error("steady53:H2InvalidOptions", "archiveTag must be a scalar name.");
end
command = "git -C " + shellQuote(root) + " rev-list -n 1 " + ...
    shellQuote(tagName);
[status, output] = system(command);
commit = lower(strtrim(string(output)));
if status ~= 0 || commit ~= lower(string(expectedCommit))
    error("steady53:H2ArchiveTagMismatch", ...
        "Archive tag peeled commit does not match the approved baseline.");
end
end

function validatePayload(payload)
required = ["result" "report" "spec"];
if ~isstruct(payload) || ~all(isfield(payload, required))
    error("steady53:H2InvalidInput", ...
        "Input MAT must contain result, report, and spec only.");
end
end

function hashes = protectedHashes(root)
paths = string([ ...
    fullfile(root, "final_steady_24a.slx")
    fullfile(root, "HeXe_property_simulink.m")
    fullfile(root, "hexe_compressor_lookup.mat")
    fullfile(root, "radiator_table.mat")
    fullfile(root, "turbine_table1.mat")
    fullfile(root, "turbine_table2.mat")
    fullfile(root, "tmp", "steady53", "task8", ...
        "run_1787582761047_bb4aa60600cc4d9e9cc15077c6f435d3", ...
        "nominal_500_report.mat")
    "/Users/ikunsredemptionmac/Downloads/不接入转子稳态模型/空间锂冷堆He-Xe布雷顿循环发电系统优化设计与运行特性分析_徐驰.pdf"]);
values = strings(numel(paths), 1);
for index = 1:numel(paths)
    values(index) = sha256File(paths(index));
end
hashes = table(paths, values, 'VariableNames', {'path', 'sha256'});
end

function hash = sha256File(filePath)
[status, output] = system("shasum -a 256 " + shellQuote(filePath));
if status ~= 0
    error("steady53:H2HashFailure", "Hash failed: %s", output);
end
parts = split(strtrim(output));
hash = lower(string(parts(1)));
end

function quoted = shellQuote(value)
quoted = "'" + replace(string(value), "'", "'\\''") + "'";
end
