...
% Contract conflict intentionally remains unresolved: the approved wording
% names turbine_expansion_ratio, while this immutable evidence exposes and
% the current read-only implementation uses turbine_lookup_expansion_ratio.
% H1a remains BLOCKED pending user approval; this task does not rename it.
...
if any(structfun(@(value) ~validFiniteRealScalar(value), inputs)) || ...
        ...
        inputs.recordedTerminalT2_K <= 0 || inputs.expansionRatio <= 0 || ...
...
try
    ...
    if startsWith(string(exception.identifier), "MATLAB:integral:")
        nonconvergence = MException( ...
            ...
            "S2 integral raised %s at the required [100 K,T1] fzero bracket. S1 local read-only result before block: phiBar=%.17g, T2s_K=%.17g, T2_K=%.17g, rootResidual_K=%.17g. reportMetricMeanT2_K=%.17g, reportMetricRelativeError=%.17g; recordedTerminalT2_K=%.17g, recordedTerminalRelativeError=%.17g. Expansion-ratio contract conflict remains BLOCKED: current immutable field turbine_lookup_expansion_ratio differs from approved wording turbine_expansion_ratio. No H1a output was published.", ...
...
lines = [ ...
    ...
    "expansionRatioContractConflict=approved wording turbine_expansion_ratio differs from current immutable evidence field turbine_lookup_expansion_ratio"
    "expansionRatioContractStatus=BLOCKED pending user approval; unresolved by H1a"
