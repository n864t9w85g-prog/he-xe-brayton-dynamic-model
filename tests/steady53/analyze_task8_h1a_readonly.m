...
sensitivity = makeSensitivityTable(inputs, settings, ...
    ...
    config, inputMat, inputMatSha256, modelPath, modelSha256, ...
    formalPropertyPath, formalPropertySha256, ...
    s2PropertySourcePath, s2PropertySourceSha256, ...
    s2EvidencePaths, s2EvidenceHashes, s2PathAudit);
...
function output = makeSensitivityTable(inputs, settings, ...
        phiBar, T2s_K, T2_K, rootResidual_K, config, ...
        inputMat, inputMatSha256, modelPath, modelSha256, ...
        formalPropertyPath, formalPropertySha256, ...
        s2PropertySourcePath, s2PropertySourceSha256, ...
        s2EvidencePaths, s2EvidenceHashes, s2PathAudit)
...
output = table(method, phiBar, T2s_K, T2_K, ...
    ...
    description);
evidence = makeEvidenceTable(config, inputMat, inputMatSha256, ...
    modelPath, modelSha256, formalPropertyPath, ...
    formalPropertySha256, s2PropertySourcePath, ...
    s2PropertySourceSha256, s2EvidencePaths, s2EvidenceHashes, ...
    settings, s2PathAudit);
output = [output evidence];
end

function output = makeEvidenceTable(config, inputMat, inputMatSha256, ...
        modelPath, modelSha256, formalPropertyPath, ...
        formalPropertySha256, s2PropertySourcePath, ...
        s2PropertySourceSha256, s2EvidencePaths, s2EvidenceHashes, ...
        settings, s2PathAudit)
rowCount = 3;
repeatText = @(value) repmat(string(value), rowCount, 1);
repeatValue = @(value) repmat(value, rowCount, 1);
runId = repeatText(config.runId);
inputMat = repeatText(inputMat);
inputMatSha256 = repeatText(inputMatSha256);
formalModel = repeatText(modelPath);
modelSha256 = repeatText(modelSha256);
formalProperty = repeatText(formalPropertyPath);
formalPropertySha256 = repeatText(formalPropertySha256);
s2PropertySource = repeatText(s2PropertySourcePath);
s2PropertySourceSha256 = repeatText(s2PropertySourceSha256);
if settings.s2PropertyVariant == "schemeA"
    s2ApprovedH2aCsv = repeatText(s2EvidencePaths(1));
    s2ApprovedH2aCsvSha256 = repeatText(s2EvidenceHashes(1));
    s2ApprovedH2aTxt = repeatText(s2EvidencePaths(2));
    s2ApprovedH2aTxtSha256 = repeatText(s2EvidenceHashes(2));
    schemeADefinition = repeatText( ...
        "ignoreHePureThirdVirialBeforeCurrentMixingRule");
else
    s2ApprovedH2aCsv = repeatText("");
    s2ApprovedH2aCsvSha256 = repeatText("");
    s2ApprovedH2aTxt = repeatText("");
    s2ApprovedH2aTxtSha256 = repeatText("");
    schemeADefinition = repeatText("none");
end
s2PhiVariant = repeatText(settings.s2PropertyVariant);
s2PhiScope = repeatText("only H1a-S2 phi integrand");
etaCp1Cp2HeldFixed = repeatValue(true);
s2IntegrationCompleted = repeatValue(true);
s2RootConverged = repeatValue(true);
s2PathAuditSampleCount = repeatValue(s2PathAudit.sampleCount);
s2PathAuditMinCpMass_J_kgK = repeatValue( ...
    s2PathAudit.minCpMass_J_kgK);
s2PathAuditMinCvMass_J_kgK = repeatValue( ...
    s2PathAudit.minCvMass_J_kgK);
s2PathAuditMinGamma = repeatValue(s2PathAudit.minGamma);
s2PathAuditMaxGamma = repeatValue(s2PathAudit.maxGamma);
s2PathAuditMinPhi = repeatValue(s2PathAudit.minPhi);
s2PathAuditMaxPhi = repeatValue(s2PathAudit.maxPhi);
s2PathAuditAllPhysical = repeatValue(s2PathAudit.allPhysical);
s2PathAuditFormalGlobalProof = repeatValue( ...
    s2PathAudit.formalGlobalProof);
slxLoadedOrSimulated = repeatValue(false);
h1bExecuted = repeatValue(false);
authorizesRepair = repeatValue(false);
formalModelPromotion = repeatValue(false);
output = table(runId, inputMat, inputMatSha256, formalModel, ...
    modelSha256, formalProperty, formalPropertySha256, ...
    s2PropertySource, s2PropertySourceSha256, s2ApprovedH2aCsv, ...
    s2ApprovedH2aCsvSha256, s2ApprovedH2aTxt, ...
    s2ApprovedH2aTxtSha256, s2PhiVariant, s2PhiScope, ...
    schemeADefinition, etaCp1Cp2HeldFixed, s2IntegrationCompleted, ...
    s2RootConverged, s2PathAuditSampleCount, ...
    s2PathAuditMinCpMass_J_kgK, s2PathAuditMinCvMass_J_kgK, ...
    s2PathAuditMinGamma, s2PathAuditMaxGamma, s2PathAuditMinPhi, ...
    s2PathAuditMaxPhi, s2PathAuditAllPhysical, ...
    s2PathAuditFormalGlobalProof, slxLoadedOrSimulated, ...
    h1bExecuted, authorizesRepair, formalModelPromotion);
end
