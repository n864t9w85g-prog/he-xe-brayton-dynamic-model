...
verifyTrue(testCase, all(isfield(manifest, ...
    ["name", "block", "port", "fluid", "kind", "constant"])));
verifyTrue(testCase, all(ismember(string({manifest.kind}), ...
...
massFlowNames = sort([ ...
    ...
    "lithium_mdot_ihx"; "turbine_lookup_mass_flow"]);
verifyEqual(testCase, ...
    sort(string({manifest(string({manifest.kind}) == "massFlow").name}).'), ...
    massFlowNames);
...
verifyManifestRow(testCase, manifest, "reactor_inlet_T", ...
    "final_steady_24a/IHX", 1);
reactorInletRow = string({manifest.name}) == "reactor_inlet_T";
verifyEqual(testCase, string(manifest(reactorInletRow).fluid), ...
    "Lithium");
verifyEqual(testCase, string(get_param( ...
    "final_steady_24a/IHX/T_ho", "Name")), "T_ho");
verifyEqual(testCase, str2double(get_param( ...
    "final_steady_24a/IHX/T_ho", "Port")), 1);
verifyConnectedPorts(testCase, ...
    "final_steady_24a/IHX", 1, ...
    "final_steady_24a/reactor", 1);
...
verifyManifestRow(testCase, manifest, "turbine_expansion_ratio", ...
    "final_steady_24a/TAC/Compressor", 1);
compressorPressureRatio = ...
    "final_steady_24a/TAC/Compressor/2-D Lookup" + newline + "Table3";
verifyEqual(testCase, string(get_param( ...
    "final_steady_24a/TAC/Compressor/r", "Name")), "r");
verifyEqual(testCase, str2double(get_param( ...
    "final_steady_24a/TAC/Compressor/r", "Port")), 1);
verifyFeedsPort(testCase, compressorPressureRatio, ...
    "final_steady_24a/TAC/Compressor/r", 1);
verifyConnectedPorts(testCase, ...
    "final_steady_24a/TAC/Compressor", 1, ...
    "final_steady_24a/TAC/Turbine", 2);
...
function verifyConnectedPorts( ...
        testCase, sourceBlock, sourcePort, destinationBlock, destinationPort)
sourcePorts = get_param(sourceBlock, "PortHandles");
destinationPorts = get_param(destinationBlock, "PortHandles");
line = get_param(sourcePorts.Outport(sourcePort), "Line");
actualDestinationPorts = get_param(line, "DstPortHandle");
verifyTrue(testCase, any(actualDestinationPorts == ...
    destinationPorts.Inport(destinationPort)), ...
    sourceBlock + " out" + sourcePort + " does not feed " + ...
    destinationBlock + " in" + destinationPort);
end
