...
def run_compressor(config: CompressorInput) -> CompressorRun:
    ...
    for vovcr in config.vovcr:
        ...
        if point.status in (
            ...
            diagnostics.append(f"VOVCR={vovcr:.2f}: {point.message}")
            continue
        # Listing labels 21-22 interpolate RANGE from the current second-pass
        # throat Mach number before testing EWF against RANGE*WCHK.
        point_surge_flow = (
            _interpolate_1d(point.throat_mach, DIFLEM, FLRNG) * choke_flow
        )
        if point.equivalent_flow < point_surge_flow:
        ...
        if surge_flow is None:
            surge_flow = point_surge_flow
        points.append(point)
        if point.incidence_choke or point.throat_choke:
            break

    if surge_flow is None:
        diagnostics.append("No second-pass point lies inside the usable range")
